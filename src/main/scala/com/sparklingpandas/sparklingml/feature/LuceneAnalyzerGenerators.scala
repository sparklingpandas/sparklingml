...
private[sparklingpandas] object LuceneAnalyzerGenerators extends CodeGenerator {
  ...
  def writeScalaCode(code: String, file: String) = {
  ...
  def writePythonCode(code: String, file: String) = {
  ...
  def main(args: Array[String]): Unit = {
    ...
    List((testCode, testCodeFile), (transformerCode, transformerCodeFile)).foreach {
      case (code: String, file: String) =>
        writeScalaCode(code, file)
    }
    writePythonCode(pyCode, pyCodeFile)
  }
  ...
  def generateStopwordStage(cls: Class[_],
    constructorParametersSizes: List[Int],
    clsShortName: String,
    clsFullName: String): (String, String, String) = {
...
lled through this API
  ...
  def generateForClass(cls: Class[_]): (String, String, String) = {
    ...
    if (isStopWordAnalyzer && charsetConstructorSizes.contains(1)) {
      ...
      generateStopwordStage(cls, constructorParametersSizes, clsShortName, clsFullName)
    } else if (constructorParametersSizes.contains(0) &&
      javaConstructorParametersSizes.contains(0)) {
      generateZeroArgStage(clsShortName, clsFullName)
    } else {
      ("", s"""
